...
use actix::prelude::*;
use actix_demo::ws::{self, WsMsg, WS_ADDRS};
use actix_web::{fs, http, server, App, AsyncResponder, FutureResponse, HttpRequest, HttpResponse};
    ...
    fn handle(&mut self, msg: Msg, _ctx: &mut Self::Context) -> Self::Result {
        ...
        std::thread::spawn(move || {
            (0..val)
                ...
                .map(|i| {
                    let value = sync_fn(i);
                    let lock = WS_ADDRS.read().unwrap();
                    let addr = id.and_then(|id| lock.get(&id));

                    if let Some(addr) = addr {
                        addr.do_send(WsMsg(value));
                    }
                })
                .count()
        });
...
fn main() {
    ...
    server::new(move || {
        ...
        vec![
            App::new()
                .prefix("/ws")
                .resource("/", |r| r.method(http::Method::GET).f(ws::ws_index))
                .boxed(),
